...
export default function EngineerProjectListPage() {
    ...
    <div className={styles.page}>
      <div className={styles.headerRow}>
        <div>
          <h1>Engineer</h1>
          <p>
            Shop Drawing, Material Approval, Metode Pekerjaan, Sertifikasi Pekerjaan, dan Daftar
            Peralatan. Pilih project untuk mulai mengelola.
          </p>
        </div>
      </div>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {projects.map((project) => {
                const pending = project._count
                  ? Object.values(project._count).reduce((a, b) => a + b, 0)
                  : 0;

                return (
                  <tr key={project.id}>
                    <td>
                      <Link href={`/civil/project/engineer/${project.id}`} className={styles.rowLink}>
                        {project.namaProject}
                      </Link>
