...
export default function MeetingProjectListPage() {
  ...
  return (
    <div className={styles.page}>
      <div className={styles.headerRow}>
        <div>
          <h1>Meeting Progress</h1>
          <p>Meeting, Dokumentasi Meeting, dan MOM. Pilih project untuk mulai mengelola.</p>
        </div>
      </div>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {projectsTampil.map((project) => (
                <tr key={project.id}>
                  <td>
                    <Link href={`/civil/project/meeting/${project.id}`} className={styles.rowLink}>
                      {project.namaProject}
                    </Link>
