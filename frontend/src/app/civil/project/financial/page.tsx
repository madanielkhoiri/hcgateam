...
export default function FinancialProjectListPage() {
    ...
    <div className={styles.page}>
      <div className={styles.headerRow}>
        <div>
          ...
          <p>Opname Pekerjaan. Pilih project untuk mulai mengelola.</p>
        </div>
      </div>
          ...
          <table className={styles.table}>
            ...
            <tbody>
              {projectsTampil.map((project) => {
                const pending = project.pendingFinancial ?? 0;

                return (
                  <tr key={project.id}>
                    <td>
                      <Link href={`/civil/project/financial/${project.id}`} className={styles.rowLink}>
                        {project.namaProject}
                      </Link>
