...
import { CalendarClock, Download, Plus, Search, Ticket, Trash2, X } from 'lucide-react';
...
export default function TiketPage() {
  ...
  async function muat() {
    try {
      setError('');
      const hasil = await transportApi.tiket.daftarAdmin({
        cari: searchDebounced || undefined,
  ...
  useEffect(() => {
    if (siap) void muat();
  }, [siap, searchDebounced, bulan, tahun, halaman]);

  // Balik ke halaman 1 tiap kali pencarian/filter berubah.
  useEffect(() => {
    setHalaman(1);
  }, [searchDebounced, bulan, tahun]);
    ...
    <section>
      ...
      <div className={styles.tablePanel}>
        <div className={styles.tableTitle}>
          ...
          <span>Total {totalData} data, ditampilkan {data.length} per halaman</span>
        </div>
