...
import { Bus, CalendarClock, Eye, Pencil, Plus, Trash2, UserRound, X } from 'lucide-react';
...
export default function TravelPage() {
  ...
  const [jadwalHalaman, setJadwalHalaman] = useState(1);
  ...
  async function muat() {
    try {
      ...
      const [d, j] = await Promise.all([
        transportApi.travel.daftarDriver(),
        transportApi.travel.daftarJadwalAdmin({
          status: jadwalStatusFilter || undefined,
          bulan: jadwalBulan || undefined,
          tahun: jadwalTahun || undefined,
          halaman: jadwalHalaman,
          ukuranHalaman: UKURAN_HALAMAN,
        }),
      ]);
      setDriverList(d);
      setJadwalList(j.data);
      setJadwalTotal(j.total);
    } catch (err) {
  ...
  useEffect(() => {
    if (siap) void muat();
  }, [siap, jadwalStatusFilter, jadwalBulan, jadwalTahun, jadwalHalaman]);

  // Balik ke halaman 1 tiap kali filter jadwal berubah.
  useEffect(() => {
    setJadwalHalaman(1);
  }, [jadwalStatusFilter, jadwalBulan, jadwalTahun]);
    ...
    <section>
      ...
      <div className={styles.tablePanel}>
        ...
        >
          <div>
            <h3>Jadwal Travel</h3>
            <span>Total {jadwalTotal} jadwal, ditampilkan {jadwalList.length} per halaman</span>
          </div>
        ...
        <PaginationBar
          halaman={jadwalHalaman}
          totalHalaman={hitungTotalHalaman(jadwalTotal, UKURAN_HALAMAN)}
          onGanti={setJadwalHalaman}
        />
      </div>
