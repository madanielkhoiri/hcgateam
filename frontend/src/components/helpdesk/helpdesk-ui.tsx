...
// ==================================================
// FILE: frontend/src/components/helpdesk/helpdesk-ui.tsx
// FUNGSI: Komponen tampilan bersama seluruh halaman Helpdesk Center
// ==================================================

import { AlertCircle, CheckCircle2, Info, Inbox, X } from 'lucide-react';
import type { ReactNode } from 'react';
import styles from '@/app/hc/helpdesk/helpdesk.module.css';
import {
...
export function BadgeStatusTiket({ nilai }: { nilai: StatusTiketHelpdesk }) {
  return (
    <span className={`${styles.badge} ${styles[nadaStatusHelpdesk(nilai)]}`}>
      {LABEL_STATUS_HELPDESK[nilai]}
    </span>
  );
}

export function BadgeLevelTiket({
  nilai,
}: {
  nilai: LevelTiketHelpdesk | null;
}) {
  if (!nilai) {
    return <span style={{ color: '#8494a9', fontSize: 11 }}>-</span>;
  }

  return (
    <span className={`${styles.badge} ${styles[nadaLevelHelpdesk(nilai)]}`}>
      {LABEL_LEVEL_HELPDESK[nilai]}
    </span>
  );
}
